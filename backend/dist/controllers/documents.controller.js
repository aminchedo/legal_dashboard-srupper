...
async function statistics(_req, res) {
    try {
        const stats = await document_service_1.documentService.getStatistics();
...
async function tags(_req, res) {
    try {
        const tags = await document_service_1.documentService.getTags();
        return res.json({ tags });
    }
    catch (error) {
        logger_1.logger.error('Failed to get tags', error);
        return res.status(500).json({
