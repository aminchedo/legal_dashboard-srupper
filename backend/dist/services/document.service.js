...
class DocumentService {
    ...
    async getStatistics() {
        try {
            ...
            const totalCompleted = db.prepare('SELECT COUNT(*) as count FROM documents WHERE status = ?').get('completed');
            const totalProcessing = db.prepare('SELECT COUNT(*) as count FROM documents WHERE status = ?').get('processing');
            const totalFailed = db.prepare('SELECT COUNT(*) as count FROM documents WHERE status = ?').get('failed');
            ...
            logger_1.logger.error('Failed to get document statistics', error);
            throw error;
        }
    }
    async getTags() {
        try {
            const db = this.db;
            const documents = db.prepare('SELECT keywords FROM documents WHERE keywords IS NOT NULL').all();
            ...
            logger_1.logger.error('Failed to get document tags', error);
            throw error;
        }
    }
