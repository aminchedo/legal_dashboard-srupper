            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-4 mt-1 text-sm text-gray-500">
                <div className="flex items-center gap-1">
                  <Calendar size={14} />
                  {format(new Date(document.createdAt), 'yyyy/MM/dd', { locale: faIR })}
                </div>
                <div className="flex items-center gap-1">
                  <BookOpen size={14} />
                  {document.wordCount.toLocaleString('fa-IR')} کلمه
                </div>
                <div className="flex items-center gap-1">
                  <Clock size={14} />
                  {document.readingTime} دقیقه مطالعه
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div className="relative group">
                ...
                <div className="absolute bottom-full left-0 mb-1 bg-white border border-gray-200 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                  <button
                    onClick={() => onExport(document.id, 'pdf')}
                    className="block w-full text-left px-4 py-2 hover:bg-gray-50 border-b border-gray-100"
                  >
                    PDF
                  </button>
                  <button
                    onClick={() => onExport(document.id, 'docx')}
                    className="block w-full text-left px-4 py-2 hover:bg-gray-50 border-b border-gray-100"
                  >
                    Word
                  </button>
                  <button
                    onClick={() => onExport(document.id, 'txt')}
