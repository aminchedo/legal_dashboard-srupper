...
try:  # Soft dependency for type hints; runtime required for actual usage
    import requests
...
except Exception:  # pragma: no cover - only for static analysis if requests isn't installed
...
@dataclass
class ProxyStats:
    """Statistics collected for a single proxy.

    Attributes
    - successes: Number of successful requests
    - failures: Number of failed requests
    - total_response_time_s: Cumulative response time in seconds
    - last_error: Text of last error (if any)
    - last_used_ts: Last epoch timestamp when proxy was used
    """
...
class ResilientProxyRotator:
    """Resilient proxy rotator with intelligent blacklisting and monitoring.

    Core features
    - Circular rotation using deque
    - Temporary blacklist with cooldown and auto-rehabilitation
    - Robust retry logic with proxy switching on failures
    - Detailed logging and per-proxy performance statistics

    Parameters
    - proxies_list: List of proxy URLs, including authentication if required.
                    Example: "http://username:password@host:port"
    - cooldown_period: Cooldown in seconds before a blacklisted proxy is retried.

    Notes
    - Default timeouts: connect=15s, read=30s
    - HTTP and HTTPS traffic will be routed via the same proxy string by default.
    """
    ...
    def __init__(self, proxies_list: List[str], cooldown_period: int = 300) -> None:
        ...
        # Encourage sane default logging in absence of application-level config
    ...
    def _rotate_proxy(self) -> Optional[str]:
        """Rotate and return the next proxy in the deque.
        ...
        current = self._proxies[0]
        return current

    # -------------------- Proxy management --------------------
    def get_proxy(self) -> Optional[str]:
        """Return the next available proxy not currently on cooldown.

        Strategy
        - Scan through the entire deque once, rotating each step to maintain
          circular order.
        - Prefer the healthiest proxy among eligible ones based on success rate
          and average response time.
        - If all proxies are on cooldown, return None.
        """
        if not self._proxies:
            return None
        ...
        for _ in range(num_proxies):
            ...
            if until_ts is not None and now >= until_ts:
                # Cooldown expired; rehabilitate for consideration
                del self._blacklist_until_ts[candidate]
                self.logger.info(
                    "Rehabilitated proxy after cooldown: %s", candidate)

            # Score candidate by health (higher is better)
            candidate_stats = self._proxy_stats.setdefault(
                candidate, ProxyStats())
            success_rate = candidate_stats.success_rate()
            avg_time = candidate_stats.average_response_time()
            # Favor higher success rate and lower latency. If no successes yet,
            # average_response_time returns inf, making score very low.
    ...
    def blacklist_proxy(self, proxy: str, reason: str = "", error: Optional[BaseException] = None) -> None:
        """Temporarily blacklist a proxy with a cooldown.

        The proxy will be eligible for use again after the cooldown period.
        """
        if not proxy:
            return
    ...
    def make_request(
        ...
        """Execute an HTTP GET with proxy rotation and robust error handling.

        Parameters
        - url: Target URL
        - session: requests.Session for persistence (cookies/headers). If None, a
                   temporary session is created for the call.
        - max_retries: Maximum attempts per request. Different proxy is used for
                       each attempt when available.
        - request_kwargs: Optional dict passed to Session.get (e.g., headers,
                          params, timeout override). If 'timeout' is not provided,
                          the instance default timeouts are used.

        Returns
        - Response on success, else None if all retries exhausted or no proxy.
        """
        ...
        # Optional exponential backoff controls (in seconds); filtered out before request
        ...
        while attempts < max_retries:
            ...
            try:
                ...
                # Proxy-specific handling (e.g. 407 auth)
                ...
                try:
                    ...
                    # Consider proxy issues for 5xx as transient proxy failures
                ...
                # Optional backoff before next retry
                if backoff_base_s > 0:
                ...
                if backoff_base_s > 0:
                    ...
                    time.sleep(max(0.0, sleep_s))
                continue
            except Exception as unexpected:  # Robustness: catch-all
                elapsed = time.time() - start_ts
                self._record_failure(
    ...
    def get_status(self) -> Dict[str, Any]:
        """Return current monitoring snapshot including proxy stats and blacklist."""
        now = time.time()
    ...
    # -------------------- Optional advanced/dynamic features --------------------
    def set_timeouts(self, connect_timeout_s: float = 15.0, read_timeout_s: float = 30.0) -> None:
        """Update default connect/read timeouts for requests."""
    ...
    def set_cooldown(self, cooldown_period_s: int) -> None:
        """Update cooldown period for blacklisted proxies."""
    ...
    def add_proxies(self, new_proxies: List[str]) -> None:
        """Add new proxies at runtime (duplicates are ignored)."""
        added = 0
    ...
    def remove_proxy(self, proxy: str) -> None:
        """Remove a proxy from rotation and stats."""
        try:
