...
async function extractAmarData(page) {
  const data = await page.evaluate(() => {
    ...
    return {
      ...
      stats: getTexts('.stat, .number, .kpi, .indicator, table td'),
...
async function scrapeAmar() {
  const logger = createLogger('amar-scrape');
  const url = 'https://www.amar.org.ir';
  const result = { target: 'amar.org.ir', url, status: 'INIT', screenshot: null, dataFile: null, htmlFile: null, timings: {} };
  ...
  for (let attempt = 1; attempt <= scrapingConfig.maxRetries; attempt++) {
    ...
    try {
      ...
      const shotPath = screenshotPath('amar-homepage');
      await page.screenshot({ path: shotPath, fullPage: true });
      result.screenshot = shotPath;
      logger.log(`Screenshot saved: ${shotPath}`);

      const html = await page.content();
      const htmlFile = saveHTML('amar-homepage', html);
      result.htmlFile = htmlFile;
      logger.log(`HTML saved: ${htmlFile}`);

      const data = await extractAmarData(page);
      const dataFile = saveJSON('amar-data', data);
...
if (require.main === module) {
  scrapeAmar().then(() => {}).catch((e) => { console.error(e); process.exit(1); });
}

module.exports = { scrapeAmar };